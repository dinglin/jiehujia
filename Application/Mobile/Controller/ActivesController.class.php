...
class ActivesController extends HomeController {
	...
	public function index($p = 1,$srh = ""){
		...
		$uid = I("uid");
		$club_id = session('main_club_id');
		
		$this->build_search($srh,$club_id);
		
		$where = $this->build_where($club_id);
		
		if($uid){
		    $club = D("Club_user")->where("w");
		    //$where[] = $uid;
		}
	    ...
	    if($p==1){
	        ...
	        //分页
	        $this->build_pages($p,$count,$club_id);
	...
	private function build_where($club_id){
	    $where = " status=1";
	    if($club_id){
	        $where.=" AND club_id=".$club_id;
	    }
	    if($this->search_m){
	        if($this->search_m=="w"){
	            $w = date('w');
	...
	private function build_pages($p,$total,$club_id,$pagesize=self::PAGE_SIZE){
	    $max_page = ceil($total/$pagesize);
	    $url_param = "";
	    if($club_id){
	        $url_param = "cid=".$club_id."&";
	    }
	    if($p==1){
	        $pre_page="";
	    }else{
	        $pre_page= U("Actives/index?{$url_param}p=".($p-1)."&srh=".$this->build_search_param($this->search_m,$this->search_d,$this->search_s));
	    }
	    
	    if($p<$max_page){
	        $next_page= U("Actives/index?{$url_param}p=".($p+1)."&srh=".$this->build_search_param($this->search_m,$this->search_d,$this->search_s));;
	    }else{
	        $next_page="";
	    }
	    $this->pages = array("pre_page"=>$pre_page,"next_page"=>$next_page,"total_page"=>$max_page,"now_page"=>$p,"total_num"=>$total);
	}
	private function build_search($search,$club_id){
	    $url_search = "p=1";
	    if($club_id){
            $url_search.="&cid=".$club_id;
        }
	    if($search){
	        $se = explode("-", $search);
	    ...
	    foreach($c_month as &$m){
	        $m['url'] = U("Actives/index?{$url_search}&srh=".$this->build_search_param($m['key'],$this->search_d,$this->search_s));
	    }
	    $this->c_month = $c_month;
	    
	    $c_days = $this->get_const_days();
	    foreach($c_days as &$d){
	        $d['url'] = U("Actives/index?{$url_search}&srh=".$this->build_search_param($this->search_m,$d['key'],$this->search_s));
	    }
	    $this->c_days = $c_days;
	    
	    $this->c_search = U("Actives/index?{$url_search}&srh=".$this->build_search_param($this->search_m,$this->search_d,""));
	...
	private function build_nextpage_ajax_html($actives,$nowpage){
	    ...
	    foreach($actives as $at){
	        $content .= '<article onclick="javascript:">';
	        $content .= '<a class="block" href="'.U('Actives/detail?id='.$at['active_id']).'">';
	        $content .= '<div class="pro-pic" style="background-image: url('.img_size_url($at['list_pic'],196,176).')"></div>';
	        $content .= '<div class="pro-info">';
	        $content .= '<h2>'.$at['title'].'</h2>';
	        $content .= '<p>';
	        $content .= '<span>'.$at['club']['club_name'].'</span>';
	        $content .= '</p>';
	        $content .= '<p>';
	        $content .= '<span class="fr pro-value"><small>¥</small>'.$at['price'].' </span>';
	        $content .= '<span>时间：'.date('m月d日',$at.start_time).'~'.date('m月d日',$at.end_time).'（'.$at['days'].'天）</span>';
	        $content .= '</p>';
	        $content .= '<p>';
	        $content .= '<span>地点 ：'.$at['destination'].' </span>';
	        $content .= '</p>';
	        $content .= '</div>';
	        $content .= '</a>';
	        $content .= '</article>';
	...
	public function detail($id = 0){
		...
		if(empty($info['imgs'])){
		    $info['imgs'][] = array("img_src"=>$info['list_pic']);
		}
		//处理图片尺寸
