...
class UserController extends HomeController {
	...
	public function index(){
		...
		$member = M("Member")->where(array("uid"=>$user['uid']))->find();
		$this->assign('member', $member);
		$umember = M("UcenterMember")->where(array("id"=>$user['uid']))->find();
		$this->assign('umember', $umember);
                //menu显示
	...
	public function login($username = '', $password = '', $verify = ''){ 
	    ...
	    $this->assign("active_id",I('active_id'));
		...
		if(IS_POST){ //登录验证
			...
			if($login_open && !check_verify($verify)){
			    //$this->error('验证码输入错误！');
			}
			...
			if(0 < $uid){ //UC登录成功
				...
				if($Member->login($uid)){ //登录用户
                    ...
                    if($active_id){
                        $this->success('登录成功！',U('User/tobuy?id='.$active_id));
                    }else{
		    ...
		    $this->assign("FOOTER_CLOSE",true);
		    $this->assign("LOGIN_OPEN",$login_open);
    ...
    public function profile(){
        ...
        if ( IS_POST ) {
            ...
            $profile['sex'] = $profile['sex']==2?"女":"男";
            $this->assign("member",$profile);
     ...
     * @return boolean
     */
    public function is_ext_login_default(){
        $result = false;
        $user = session('user_auth');
    ...
    public function tobuy($id){
        $id = intval($id);
        if ( !is_login() ) {
            $this->error( '您还没有登陆',U('User/login?active_id='.$id) );
        }
        ...
        $this->assign("PAGE_TITLE",'预定-'.$info['title']);
    ...
    public function orderlist(){
        ...
        foreach ($list as $key => $value) {
            ...
            $ids_orders[] = $value['order_id'];
        }
        if($ids_orders){
            $goods_list = M('Order_goods')->where('order_id in ('.implode(",",$ids_orders).')')->select();
            $data_goods = array();
            $ids_activity = array();
            foreach($goods_list as $good){
                $ids_activity[] = $good['goods_id'];
            }
            $activites = M('Actives')->where('active_id in ('.implode(",",$ids_activity).')')->getField("active_id,list_pic,start_time,end_time,destination,days");
            foreach($goods_list as $good){
                $good['active_info'] = $activites[$good['goods_id']];
                $data_goods[$good['order_id']][] = $good;
            }
            foreach ($list as $key => $value) {
                $list[$key]['goods_list'] = $data_goods[$value['order_id']];
            }
        }
        $this->assign('_list', $list);
        //menu显示
