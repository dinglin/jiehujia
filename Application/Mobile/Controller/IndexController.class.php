...
class IndexController extends HomeController {
    ...
    public function index(){
        ...
        //推荐活动
        $actives = $this->recom_actives($club_id);
        ...
        //搜索
        $this->base_search = U("Actives/index?{$url_search}&srh=0");
        ...
        $page_title = $this->get_club_info($club_id);
        $this->assign("PAGE_TITLE",$page_title?$page_title:'性感牛肉');
        $this->display();
    }
    
    private function get_club_info($club_id){
        if($club_id){
            $club = M("Club")->where(array("club_id"=>$club_id))->getField("club_id,club_name");
            return $club[$club_id];
        }
        return null;
    }
     ...
     * @return unknown
     */
    private function recom_actives($club_id){
        
        $actives = D("Actives")->recom_actives(6,$club_id);
        $club_ids = array();
