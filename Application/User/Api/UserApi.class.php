...
class UserApi extends Api{
    ...
    public function register_ext($openid, $exttype, $nickname='', $gender = '',$headpic=''){
        return $this->model->register_ext($openid, $exttype, $nickname, $gender,$headpic);
    }
    /**
     * 用户登录认证
     * @param  string  $username 用户名
    ...
    public function login_ext($open_id,$login_type){
        return $this->model->login_ext($open_id, $login_type);
    }
    /**
     * 获取用户信息
    ...
    public function update_profile($post){
        $fields = array("nickname","phone","sex","birthday");
        $data = array();
        foreach($fields as $field){
            $data[$field] = $post[$field];
        }
         return M("Member")->data($data)->where(array("uid"=>$post["uid"]))->save();
    }
}
