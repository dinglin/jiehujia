! function(a) {
	var c = "createTouch" in document || "ontouchstart" in a || 0,
		...
		i = function(a) {
			...
			try {
				if ("div" == this.container.nodeName.toLowerCase() ? (this.element = this.container, this.container = this.element.parentNode) : this.element = this.container.getElementsByTagName("div")[0], "undefined" == typeof this.element) throw new Error('Can\'t find "ul"');
	...
	i.prototype = {
		...
		$E: function(a, b, c) {
			...
			for (f = 0, g = e.length; g > f; f++) new RegExp("(?:^|\\s+)" + a + "(?:\\s+|$)").test(e[f].className) && d.push(e[f]);
			return d
		},
		isIE: function() {
			return !-[1]
				...
				switch (a) {
					...
					case "opacity":
						return "opacity" in document.body.style ? "opacity" : {
							...
							set: function(a, b) {
								a.style.filter = "alpha(opacity=" + 100 * b + ")", a.style.zoom = 1
							}
						};
					default:
						for (b = a.split("-"), c = 1; c < b.length; c++) b[c] = b[c].substring(0, 1).toUpperCase() + b[c].substring(1);
						return a = b.join("")
				}
			}, b = function(b, c) {
					var d, e;
					return c = a(c), d = b.style[c], d || (e = document.defaultView && document.defaultView.getComputedStyle && getComputedStyle(b, null) || b.currentStyle || b.style, d = "string" == typeof c ? e[c] : c.get(b, e)), "auto" == d ? "" : d
				}, c = function(b, c) {
					var d, e;
					for (e in c) d = a(e), "string" == typeof d ? b.style[d] = c[e] : d.set(b, c[e])
				};
			return function(a, d) {
				return "string" == typeof d ? b(a, d) : c(a, d)
			}
		}(),
		parse_args: function(a) {
			var d, b = {}, c = Object.prototype.toString;
			if (a && "[object Object]" == c.call(a))
				for (d in this._default) b[d] = "undefined" == typeof a[d] ? this._default[d] : "[object Number]" == c.call(this._default[d]) ? parseInt(100 * parseFloat(a[d])) / 100 : a[d];
		...
		getMousePoint: function(b) {
			var g, d = y = 0,
				...
				f = document.body;
			return b || (b = a.event), a.pageYoffset ? (d = a.pageXOffset, y = a.pageYOffset) : (d = (e && e.scrollLeft || f && f.scrollLeft || 0) - (e && e.clientLeft || f && f.clientLeft || 0), y = (e && e.scrollTop || f && f.scrollTop || 0) - (e && e.clientTop || f && f.clientTop || 0)), c && b.touches.length ? (g = b.touches[0], d += g.clientX, y += g.clientY) : (d += b.clientX, y += b.clientY), {
				x: d,
				y: y
			}
		},
		...
		setup: function() {
			if (this.status = 0, this.slides = this.opt.className ? this.$E(this.opt.className, "div", this.element) : this.element.getElementsByTagName("ul"), this.length = this.slides.length, this.opt.timeout = Math.max(this.opt.timeout, this.opt.speed), this.touching = !! c, this.css3transition = !! e, this.index = this.opt.auto < 0 || this.opt.auto >= this.length ? 0 : this.opt.auto, !(this.length < 2)) {
		...
		resize: function() {
			...
			})) : (a.width = this.width * this.length + "px", a.left = -this.width * this.index + "px"), this.css(this.element, a), b = 0; b < this.length; b++) this.css(this.slides[b], {
				width: this.width + "px",
				height: this.height + "px",
		...
		slide: function(a, b) {
			var f, g, h, i, j, k, l, m, n, o, c = this.vertical ? "top" : "left",
				d = this.vertical ? "height" : "width";
			a = 0 > a ? this.length - 1 : a >= this.length ? 0 : a, b = "undefined" == typeof b ? this.opt.speed : parseInt(b), f = this.element, g = null, h = f.style, i = this, j = 0, k = parseInt(h[c]) || 0, l = -a * this[d] - k, m = Math.abs(l) < this[d] ? Math.ceil(Math.abs(l) / this[d] * b / 10) : b / 10, n = function(a, b, c, d) {
				return -c * ((a = a / d - 1) * a * a * a - 1) + b
			}, o = function() {
				m > j && !e ? (j++, h[c] = Math.ceil(n(j, k, l, m)) + "px", g = setTimeout(o, 10)) : (h[c] = -i[d] * a + "px", i.index = a, e || i._transitionend(), i.pause(), i.begin())
			}, h.WebkitTransition = h.MozTransition = h.msTransition = h.OTransition = h.transition = c + " " + 10 * m + "ms " + this.opt.fx, this.opt.before.call(this, a, this.slides[this.index]), o()
		},
		begin: function() {
			...
			}, this), this.opt.timeout), this.status = 1, void 0)
		},
		pause: function() {
		...
		prev: function(a) {
			a = "undefined" == typeof a ? a = 1 : a % this.length;
			var b = a > this.index ? this.length + this.index - a : this.index - a;
			this.slide(b)
		},
		next: function(a) {
			"undefined" == typeof a && (a = 1), this.slide((this.index + a) % this.length)
		},
		_start: function(a) {
		...
		_move: function(a) {
			var b, c, d, e, f;
			!this.scrolling || a.touches && a.touches.length > 1 || a.scale && 1 !== a.scale || (b = this.vertical ? "top" : "left", c = this.vertical ? "height" : "width", d = this.vertical ? "y" : "x", e = this.vertical ? "x" : "y", this.endPos = this.getMousePoint(a), f = this.endPos[d] - this.startPos[d], 2 === this.scrolling || Math.abs(f) >= Math.abs(this.endPos[e] - this.startPos[e]) ? (this.preventDefault(a), this.pause(), f /= !this.index && f > 0 || this.index == this.length - 1 && 0 > f ? Math.abs(f) / this[c] + 1 : 1, this.element.style[b] = -this.index * this[c] + f + "px", 0 != f && (this.scrolling = 2)) : this.scrolling = 0)
		},
		_end: function() {
			if ("undefined" != typeof this.scrolling) {
				try {
					...
					d = 0
				}(new Date - this.startTime < 250 && Math.abs(d) > .1 * this[c] || Math.abs(d) > this[c] / 2) && (0 > d && this.index + 1 < this.length || d > 0 && this.index > 0) ? d > 0 ? this.prev() : this.next() : this.slide(this.index), delete this.scrolling, delete this.startPos, delete this.endPos, delete this.startTime, this.opt.auto >= 0 && this.begin()
			}
		},
		mouseScroll: function(b) {
			if (this.opt.mouseWheel) {
				this.preventDefault(b), b = b || a.event;
				var c = b.wheelDelta || b.detail && -1 * b.detail || 0;
				c / Math.abs(c), c > 0 ? this.next() : this.prev()
			}
		},
