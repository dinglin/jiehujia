var Swiper = function(f, b) {
		...
		if (a.isTouched && !b.onlyExternal && !(u && "mousemove" == c.type)) {
			...
			if (!w) {
				if (c.assignedToSwiper) {
					...
					if (c.assignedToSwiper = !0, b.preventLinks && (a.allowLinks = !1), b.autoPlay && a.stopAutoPlay(), !u || 1 == c.touches.length) {
						...
						if (b.resistance) {
							if (0 < a.positions.current && (!b.freeMode || b.freeModeFluid)) {
								b.loop ? (c = 1, 0 < a.positions.current && (a.positions.current = 0)) : c = (2 * l - a.positions.current) / l / 2, a.positions.current = .5 > c ? l / 2 : a.positions.current * c
							}
	...
	if (f.nodeType || document.querySelectorAll && 0 != document.querySelectorAll(f).length) {
		...
		a.init = function(c) {
			var d;
			if (this.params.htmlOnOff) {
				d = $("html").width()
			} else {
				d = window.getComputedStyle(a.container, null).getPropertyValue("width")
			}
			var e = window.getComputedStyle(a.container, null).getPropertyValue("height"),
				f = parseInt(d, 10),
				g = parseInt(e, 10);
