...
//var  LT_YIDU_MERID = "302290099970001"; // test异度商户号  
var LT_YIDU_MERID = "302290047220001"; // 异度商户号  
...
/*异度支付*/
function order_ticket_yidu(requestPath,productId,branchId,canOrderToday) {
    $("#requestContextPath_yidu").val(requestPath);
    $("#productId_yidu").val(productId);
    $("#branchId_yidu").val(branchId);
    $("#today_order_yidu").val(canOrderToday);
    
    var c_time = getLocalTime();
	var jsonData = '{"function":"getCstInfo","data":{"merid":"'+LT_YIDU_MERID+'","reqtime":"'+c_time+'"},"callback":"order_ticket_yidu_callBack"}';
	try{
...
/**
* 异度登录回调 
* @param data { "retcode":"xxxxx","retdata":"xxxxx"}
*/
function order_ticket_yidu_callBack(jsonData) {
	if(jsonData!=null && jsonData!=""){
		var data=JSON.parse(jsonData);
		if(null != data.retcode) {
			if(data.retcode=='AAAAAAA') {// 成功 
				var url = contextPath+'/yidu/getUserInfos.htm';
				$.ajax({
					url : url,
					data : {"retData":data.retdata},
					type : "POST",
					beforeSend : function() {
						//lvToast(true,'加载中...',8000);
					},
					success : function(data) {
						// 如果可以
						if(data.code == '1') {
							var requestPath = $("#requestContextPath_yidu").val();
						    var productId = $("#productId_yidu").val();
						    var branchId = $("#branchId_yidu").val();
						    var canOrderToday =  $("#today_order_yidu").val();
							var url=requestPath+'/ticketorder/order_fill.htm?productId='+productId+'&branchId='+branchId+'&canOrderToday='+canOrderToday;
							window.location.href = url;
						} else {
							globe.lvToast(false,data.message,LT_LOADING_CLOSE);
						}
					},
					error:function() {
						globe.lvToast(false,"哎呀,网络不给力,请稍后再试试吧.",LT_LOADING_CLOSE);
					...
					complete:function() {
						//lvToast(true,'加载中...',LT_LOADING_CLOSE);
					}
				});
			} else {
				//lvToast(false,data.message,LT_LOADING_CLOSE);
				globe.lvToast(false,"登录取消...",LT_LOADING_CLOSE);
			}
		}
	}
}

//获取yyyy-MM-dd hh:mm:ss.S格式的时间戳
